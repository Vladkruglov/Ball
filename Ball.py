...
import time
class Ball:
    def __init__(self, canvas, paddle1, color):
        self.canvas = canvas
...
class Paddle2:
    def __init__(self, canvas, color):
        self.canvas = canvas
        self.id = canvas.create_rectangle(0, 0, 100,5, fill=color)
        self.canvas.move(self.id, 250, 25)
        self.x = 0
        self.canvas_width = self.canvas.winfo_width()
        self.canvas.bind_all('<a>', self.turn_left1)
        self.canvas.bind_all('<d>', self.turn_right1)
        self.canvas.bind_all('<p>', self.turn_right1)
    def draw(self):
        self.canvas.move(self.id, self.x, 0)
        pos = self.canvas.coords(self.id)
        if pos[0] <= 0:
